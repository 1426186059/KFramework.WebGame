...
| **2** | **C# + WebAssembly 开发** | .NET 10 WASM（Blazor / WasmBrowserApp） | 游戏逻辑全部用 C# 编写，编译为 WASM 跑在浏览器；浏览器底层能力（渲染 / 输入 / 音效 / 存储）通过一层极薄的 JS 桥（`[JSImport]` / `[JSExport]`）暴露给 C#。**主要用于跨平台**复用 C# 逻辑，本仓库即采用此路线。 |
...
> 一句话取舍：**原生 JS 在性能与启动速度上都是第一；WebAssembly / Emscripten 的核心价值是跨平台**——本引擎按方法 2 实现跨平台复用。
