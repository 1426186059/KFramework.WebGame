...
public sealed class BreakoutScene : GameScene
{
    ...
    {
        WebGL.FillText("得分", 40, 30, "bold 13px system-ui, sans-serif", "#8b949e", "left");
        WebGL.FillText(_score.ToString(), 40, 52, "bold 26px system-ui, sans-serif", "#e6edf3", "left");

        WebGL.FillText("最高", GameEngine.Width - 40, 30, "bold 13px system-ui, sans-serif", "#8b949e", "right");
        WebGL.FillText(_highScore.ToString("0"), GameEngine.Width - 40, 52, "bold 26px system-ui, sans-serif", "#f6c445", "right");

        WebGL.FillText("生命", 30, GameEngine.Height - 44, "bold 12px system-ui, sans-serif", "#8b949e", "left");
        for (int i = 0; i < _lives; i++)
        {
            WebGL.FillCircle(34 + i * 26, GameEngine.Height - 22, 8, "#ff6b6b");
        }
    }
    ...
    private void RenderBricks()
    {
        foreach (var brick in _bricks)
        {
            ...
            WebGL.RoundedRect(brick.X - brick.W / 2 + 3, brick.Y - brick.H / 2 + 3, brick.W - 6, brick.H * 0.4f, 5, "#ffffff26");
        }
        WebGL.NoShadow();
    }
    ...
    private void RenderParticles()
    {
        ...
        {
            WebGL.Alpha(p.Alpha);
            WebGL.FillCircle(p.Position.X, p.Position.Y, p.Size * p.Alpha + 0.6f, p.Color);
        }
        WebGL.Alpha(1);
    }
    ...
    {
        var p = _paddle;
        WebGL.Shadow("#4dabf7", 16);
        WebGL.RoundedRect(p.Position.X - p.Width / 2, p.Position.Y - p.Height / 2, p.Width, p.Height, 8, "#4dabf7");
        WebGL.NoShadow();
        WebGL.RoundedRect(p.Position.X - p.Width / 2 + 5, p.Position.Y - p.Height / 2 + 3, p.Width - 10, p.Height * 0.45f, 6, "#ffffff33");
    }

    private void RenderBall()
    {
        var b = _ball;
        WebGL.Shadow("#ffe066", 14);
        WebGL.FillCircle(b.Position.X, b.Position.Y, b.Radius, "#ffe066");
        WebGL.NoShadow();
        WebGL.FillCircle(b.Position.X - 2, b.Position.Y - 2, b.Radius * 0.45f, "#fff8d6");
    }
    ...
    private void RenderMenu()
    {
        ...
        WebGL.Shadow("#4dabf7", 24);
        WebGL.FillText("BREAKOUT", cx, cy - 80, "bold 64px system-ui, sans-serif", "#e6edf3", "center");
        WebGL.NoShadow();

        WebGL.FillText("基于 .NET 10 + WebAssembly 的 2D 游戏引擎", cx, cy - 22, "18px system-ui, sans-serif", "#8b949e", "center");

        if ((int)(_stateTime * 2) % 2 == 0)
            WebGL.FillText("点击或按空格键开始", cx, cy + 30, "bold 22px system-ui, sans-serif", "#ffe066", "center");

        WebGL.FillText("← → / A D 或鼠标移动挡板  ·  空格发射", cx, GameEngine.Height - 70, "15px system-ui, sans-serif", "#6e7681", "center");
    }
    ...
    private void RenderOverlay(string title, string sub, string color)
    {
        ...
        WebGL.FillText(sub, cx, cy + 22, "bold 24px system-ui, sans-serif", "#e6edf3", "center");

        if ((int)(_stateTime * 2) % 2 == 0)
            WebGL.FillText("点击或按空格键重新开始", cx, cy + 70, "16px system-ui, sans-serif", "#8b949e", "center");
    }
}
