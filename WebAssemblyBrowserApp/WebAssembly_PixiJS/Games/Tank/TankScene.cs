...
public sealed class TankScene : GameScene
{
    ...
    public override void Render()
    {
        ...
        foreach (var b in _bullets)
        {
            ...
            Pixi.FillRect(b.X, b.Y, 10, 10, c);
            Pixi.Shadow(c, 8);
            Pixi.FillRect(b.X + 1, b.Y + 1, 8, 8, "#ffffffaa");
            Pixi.NoShadow();
        }
    ...
    {
        // HUD 背景条
        Pixi.FillRect(0, 0, GameEngine.Width, HudH, "#161b22");
        Pixi.FillRect(0, HudH - 1, GameEngine.Width, 1, "#30363d");

        float left = 28;
        Pixi.FillText("坦克大战", left, 20, "bold 18px system-ui, sans-serif", "#e6edf3", "left");
        Pixi.FillText($"第 {_levelIndex + 1} / {Levels.Length} 关", left, HudH - 14, "14px system-ui, sans-serif", "#8b949e", "left");

        float cx = GameEngine.Width / 2;
        Pixi.FillText("剩余敌军", cx, 20, "13px system-ui, sans-serif", "#8b949e", "center");
        Pixi.FillText($"{_enemiesRemainingToSpawn + _enemies.Count}", cx, HudH - 16, "bold 20px system-ui, sans-serif", "#f6c445", "center");

        float rx = GameEngine.Width - 28;
        Pixi.FillText("生命", rx, 20, "13px system-ui, sans-serif", "#8b949e", "right");
        for (int i = 0; i < _lives; i++)
            Pixi.FillRect(rx - 24 - i * 26, HudH - 22, 18, 18, "#4dabf7");

        // ESC 提示
        Pixi.FillText("ESC 菜单", rx - 120, HudH - 14, "12px system-ui, sans-serif", "#6e7681", "right");
    }
    ...
    private void RenderMap()
    {
        ...
        for (int c = 0; c < Cols; c++)
        {
            ...
            {
                case TileKind.Brick:
                    Pixi.FillRect(x, y, Tile, Tile, "#8b5a2b");
                    Pixi.FillRect(x + 1, y + 1, Tile / 2 - 2, Tile / 2 - 2, "#a97142");
                    Pixi.FillRect(x + Tile / 2 + 1, y + Tile / 2 + 1, Tile / 2 - 2, Tile / 2 - 2, "#a97142");
                    break;
                case TileKind.Steel:
                    Pixi.FillRect(x, y, Tile, Tile, "#6e7681");
                    Pixi.FillRect(x + 3, y + 3, Tile - 6, Tile - 6, "#c9d1d9");
                    Pixi.FillRect(x + 3, y + 3, Tile - 6, 2, "#ffffffaa");
                    break;
                case TileKind.Water:
                    float wave = (int)((MathF.Sin(GameEngine.Instance.Time * 4 + r) + 1) * 8);
                    Pixi.FillRect(x, y, Tile, Tile, "#2f6fed");
                    Pixi.FillRect(x + 2, y + 4 + wave % 6, Tile - 4, 2, "#ffffff55");
                    Pixi.FillRect(x + 2, y + 14 + (wave * 2) % 6, Tile - 4, 2, "#ffffff33");
                    break;
                case TileKind.Eagle:
                    RenderEagle(x, y);
                    break;
    ...
    {
        Pixi.FillRect(x, y, Tile, Tile, "#30363d");
        Pixi.FillRect(x + 4, y + 4, Tile - 8, Tile - 8, "#f6c445");
        // 鹰：两条 V 形线
        Pixi.Shadow("#f6c445", 10);
        Pixi.DrawLine(x + 6, y + Tile - 8, x + Tile / 2, y + 8, 2, "#8a6d1b");
        Pixi.DrawLine(x + Tile / 2, y + 8, x + Tile - 6, y + Tile - 8, 2, "#8a6d1b");
        Pixi.NoShadow();
    }
    ...
    private void RenderTrees()
    {
        ...
        for (int c = 0; c < Cols; c++)
        {
            ...
            float y = HudH + r * Tile;
            Pixi.Alpha(0.78f);
            Pixi.FillRect(x + 2, y + 2, Tile - 4, Tile - 4, "#2ea043");
            Pixi.FillRect(x + 6, y + 6, 6, 6, "#3fb950");
            Pixi.FillRect(x + 16, y + 10, 7, 7, "#3fb950");
            Pixi.FillRect(x + 10, y + 18, 8, 8, "#3fb950");
            Pixi.Alpha(1);
        }
    }
    ...
    {
        Pixi.Shadow(t.Color, 10);
        // 主体圆角矩形
        Pixi.RoundedRect(t.X, t.Y, TankUnit.Size, TankUnit.Size, 4, t.Color);
        Pixi.NoShadow();
        // 履带阴影
        Pixi.FillRect(t.X + 1, t.Y + 2, 4, TankUnit.Size - 4, "#00000066");
        Pixi.FillRect(t.X + TankUnit.Size - 5, t.Y + 2, 4, TankUnit.Size - 4, "#00000066");
        // 炮塔（圆心）
        float cx = t.X + TankUnit.Size / 2;
        float cy = t.Y + TankUnit.Size / 2;
        Pixi.FillCircle(cx, cy, 7, t.IsPlayer ? "#e6edf3" : "#ffffffcc");
        // 炮管（朝 Dir）
        float bx, by, bw, bh;
        switch (t.Dir)
        {
        ...
        Pixi.FillRect(bx, by, bw, bh, t.IsPlayer ? "#22272e" : "#3b1b1b");
    }
    ...
    private static void RenderOverlay(string title, string sub, string color)
    {
        ...
        Pixi.FillText(title, cx, cy - 20, "bold 50px system-ui, sans-serif", color, "center");
        Pixi.NoShadow();
        Pixi.FillText(sub, cx, cy + 30, "bold 20px system-ui, sans-serif", "#e6edf3", "center");
    }
}
