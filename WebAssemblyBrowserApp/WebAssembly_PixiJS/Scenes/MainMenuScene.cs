...
public sealed class MainMenuScene : GameScene
{
    ...
    public override void Render()
    {
        ...
        // 标题
        Pixi.Shadow("#4dabf7", 22);
        Pixi.FillText("KFramework 2D Games", cx, 110, "bold 46px system-ui, sans-serif", "#e6edf3", "center");
        Pixi.NoShadow();
        Pixi.FillText("选择一个游戏开始  ·  基于 .NET 10 WebAssembly + WebGPU",
        ...
        {
            Pixi.FillText("↑ ↓ 选择  ·  回车 / 空格 / 点击 进入  ·  游戏中按 ESC 回到菜单",
        ...
        if (_demoVideo is { Id: >= 0 })
        {
            ...
            Pixi.FillText("视频纹理 · importExternalTexture", GameEngine.Width - 16, GameEngine.Height - 172,
    ...
    private void RenderBgSparkles()
    {
        for (int i = _bg.Count - 1; i >= 0; i--)
        {
            ...
            Pixi.Alpha(a);
            Pixi.FillRect(s.X, y, s.Size, s.Size, i % 3 == 0 ? "#4d6bff" : (i % 3 == 1 ? "#f6c445" : "#ff6b6b"));
        }
        Pixi.Alpha(1);
        // 生成
    ...
    private void RenderGameItem(float cx, float y, GameItem g, bool selected)
    {
        ...
        if (selected)
        {
            Pixi.Shadow(g.Accent, 28);
            Pixi.RoundedRect(x, top, w, h, 14, g.Accent + "33");
            Pixi.StrokeRect(x - 2, top - 2, w + 4, h + 4, 3, g.Accent);
            Pixi.NoShadow();
        }
        else
        {
            Pixi.RoundedRect(x, top, w, h, 14, "#161b22");
            Pixi.StrokeRect(x, top, w, h, 1.5f, "#30363d");
        }

        Pixi.FillText(g.DisplayName, x + 28, y - 8, "bold 22px system-ui, sans-serif", selected ? "#e6edf3" : "#c9d1d9", "left");
        Pixi.FillText(g.Hint, x + 28, y + 20, "14px system-ui, sans-serif", "#8b949e", "left");

        Pixi.FillText("▶", x + w - 40, y, "bold 26px system-ui, sans-serif",
