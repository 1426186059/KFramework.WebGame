...
public sealed class MainMenuScene : GameScene
{
    ...
    public override void Render()
    {
        ...
        // 标题
        Canvas2D.Shadow("#4dabf7", 22);
        Canvas2D.FillText("KFramework 2D Games", cx, 110, "bold 46px system-ui, sans-serif", "#e6edf3", "center");
        Canvas2D.NoShadow();
        Canvas2D.FillText("选择一个游戏开始  ·  基于 .NET 10 WebAssembly + Canvas2D",
        ...
        {
            Canvas2D.FillText("↑ ↓ 选择  ·  回车 / 空格 / 点击 进入  ·  游戏中按 ESC 回到菜单",
    ...
    private void RenderBgSparkles()
    {
        for (int i = _bg.Count - 1; i >= 0; i--)
        {
            ...
            Canvas2D.Alpha(a);
            Canvas2D.FillRect(s.X, y, s.Size, s.Size, i % 3 == 0 ? "#4d6bff" : (i % 3 == 1 ? "#f6c445" : "#ff6b6b"));
        }
        Canvas2D.Alpha(1);
        // 生成
    ...
    private void RenderGameItem(float cx, float y, GameItem g, bool selected)
    {
        ...
        if (selected)
        {
            Canvas2D.Shadow(g.Accent, 28);
            Canvas2D.RoundedRect(x, top, w, h, 14, g.Accent + "33");
            Canvas2D.StrokeRect(x - 2, top - 2, w + 4, h + 4, g.Accent, 3);
            Canvas2D.NoShadow();
        }
        else
        {
            Canvas2D.RoundedRect(x, top, w, h, 14, "#161b22");
            Canvas2D.StrokeRect(x, top, w, h, "#30363d", 1.5f);
        }

        Canvas2D.FillText(g.DisplayName, x + 28, y - 8, "bold 22px system-ui, sans-serif", selected ? "#e6edf3" : "#c9d1d9", "left");
        Canvas2D.FillText(g.Hint, x + 28, y + 20, "14px system-ui, sans-serif", "#8b949e", "left");

        Canvas2D.FillText("▶", x + w - 40, y, "bold 26px system-ui, sans-serif",
