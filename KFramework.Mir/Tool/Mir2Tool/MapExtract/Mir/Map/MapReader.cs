...
{
    /// <summary>
    /// 移植自 Unity 工程 Assets/Model/Script/MirGraphics/MapReader.cs
    /// 负责解析各种传奇地图二进制格式，已移除 UnityEngine 依赖。
    /// </summary>
    public partial class MapReader
    {
        public int Width, Height;
        public CellInfo[][] MapCells = Array.Empty<CellInfo[]>();
        private byte[] Bytes = Array.Empty<byte>();
        public static Dictionary<string, MapReader> MapReaderDic = new Dictionary<string, MapReader>();

        /// <summary>
        /// 地图格式名称，可用于匹配素材库类型：WemadeMir2 / ShandaMir2 / WemadeMir3 / ShandaMir3 / Custom / Unknown
        /// </summary>
        public string MapFormatName { get; private set; } = "Unknown";

        /// <summary>
        /// 地图 Type 编号，对应 LoadMapTypeX 中的 X 值
        /// </summary>
        public int MapTypeId { get; private set; } = -1;

        public static MapReader? TryGetMapInfo(string map)
        {
            if (MapReaderDic.TryGetValue(map, out var mr))
            {
                return mr;
            }
            return null;
        ...
        public MapReader(byte[] bytes)
        {
            Bytes = bytes;
            if (Bytes == null)
            {
                MirLog.LogError("加载地图失败");
                return;
            }
        ...
        private void LoadMapType0()
        {
            ...
            MapCells = new CellInfo[Width][];
            offset = 52;
            for (int x = 0; x < Width; x++)
            {
                MapCells[x] = new CellInfo[Height];
            ...
            for (int x = 0; x < Width; x++)
            {
                ...
                for (int y = 0; y < Height; y++)
                {
                    ...
                    if (MapCells[x][y].FrontAnimationFrame > 0x0F)//assuming shanda used same value not sure
                        MapCells[x][y].FrontAnimationFrame = (byte)(MapCells[x][y].FrontAnimationFrame & 0x0F);
                    offset++;
