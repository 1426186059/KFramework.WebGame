...
import { dom, gfx, GfxSource } from '../../shared.js';

/** 当前 WebGL 程序里用到的全部 attribute / uniform 位置。 */
interface LocMap {
    aPos: number;
    aUv: number;
    ...
    uUseTex: WebGLUniformLocation | null;
    uPremult: WebGLUniformLocation | null;
}

let program: WebGLProgram | null = null, quadBuf: WebGLBuffer | null = null, vao: WebGLVertexArrayObject | null = null, loc = {} as LocMap;
let advancedBlend: any = null;
...
export function ensureGL(): boolean {
    if (initialized) return !!program;
    const gl = dom.gl;
    if (!gl) { console.error('[gl] 无 WebGL 上下文'); return false; }
    // WebGL 的 create* 在上下文丢失时会返回 null，这里显式拦住，避免后续调用拿到 null 参数。
    const vs = gl.createShader(gl.VERTEX_SHADER);
    const fs = gl.createShader(gl.FRAGMENT_SHADER);
    if (!vs || !fs) { console.error('[gl] 创建着色器失败'); return false; }
    gl.shaderSource(vs, VS); gl.compileShader(vs);
    if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) console.error('[gl] VS', gl.getShaderInfoLog(vs));
    gl.shaderSource(fs, FS); gl.compileShader(fs);
    if (!gl.getShaderParameter(fs, gl.COMPILE_STATUS)) console.error('[gl] FS', gl.getShaderInfoLog(fs));
    const prog = gl.createProgram();
    if (!prog) { console.error('[gl] 创建程序失败'); return false; }
    program = prog;
    gl.attachShader(program, vs); gl.attachShader(program, fs); gl.linkProgram(program);
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) console.error('[gl] link', gl.getProgramInfoLog(program));
    ...
    if (!buf || !vaoObj) { console.error('[gl] 创建顶点缓冲 / VAO 失败'); return false; }
    quadBuf = buf;
    vao = vaoObj;
    ...
    if (!advancedBlend) console.warn('[gl] 缺少 EXT_blend_equation_advanced：screen/multiply 回退为 source-over');
    gl.enable(gl.BLEND);
 ...
 */
export function resolveSource(tex: number): GfxSource | null {
    const t = gfx.textures.get(tex);
    if (t) return { tex: t.tex, w: t.w, h: t.h };
    const o = gfx.offscreens.get(tex);
    if (o && o.tex) return { tex: o.tex, w: o.w, h: o.h };
    return null;
}

/** 绑定当前绘制目标（离屏 FBO 或主画布）并设置 viewport。 */
function bindTarget(): void {
    const gl = dom.gl;
    gl.bindVertexArray(vao);
...
function targetWH(): { isMain: boolean; W: number; H: number } {
 ...
 * @param W 目标宽度
 */
const clipX = (px: number, W: number): number => px / W * 2 - 1;
 ...
 */
const clipY = (py: number, H: number, isMain: boolean): number => isMain ? (1 - py / H * 2) : (py / H * 2 - 1);
...
export function applyBlend(): void {
    ...
    switch (gfx.blendOp) {
        ...
        case 'screen':
            if (advancedBlend) { gl.blendFunc(gl.ONE, gl.ZERO); gl.blendEquation(advancedBlend.SCREEN_KHR); }
            else basic();
            break;
        case 'multiply':
            if (advancedBlend) { gl.blendFunc(gl.ONE, gl.ZERO); gl.blendEquation(advancedBlend.MULTIPLY_KHR); }
            // 没有扩展时用核心 WebGL 也能表达乘法：dst * src
            else { gl.blendEquation(gl.FUNC_ADD); gl.blendFunc(gl.DST_COLOR, gl.ZERO); }
            break;
        default:
            basic(); break;
    }
 ...
 */
export function drawTexQuad(src: GfxSource, sx: number, sy: number, sw: number, sh: number, dx: number, dy: number, dw: number, dh: number, alpha: number, premult: boolean): void {
    const gl = dom.gl;
    if (!gl || !src) return;
    ensureGL();
    bindTarget();
 ...
 * @param premult 源是否已预乘 alpha
 */
export function drawTexQuadTransform(src: GfxSource, sx: number, sy: number, sw: number, sh: number, m11: number, m12: number, m21: number, m22: number, m31: number, m32: number, alpha: number, premult: boolean): void {
    const gl = dom.gl;
    if (!gl || !src) return;
    ensureGL();
    bindTarget();
    ...
    const tf = (x: number, y: number): [number, number] => [m11 * x + m21 * y + m31, m12 * x + m22 * y + m32];
 ...
 */
export function drawSolidQuad(x: number, y: number, w: number, h: number, argb: number): void {
    const gl = dom.gl;
    if (!gl) return;
    ensureGL();
    bindTarget();
 ...
 * @param argb 颜色，32 位 ARGB
 */
export function drawSolidQuadPts(pts: number[][], argb: number): void {
    const gl = dom.gl;
    if (!gl || !pts || pts.length < 4) return;
    ensureGL();
    bindTarget();
    applyBlend();
    gl.useProgram(program);
    ...
    const { isMain, W, H } = targetWH();
    const v: number[] = [];
    for (let i = 0; i < 4; i++) { v.push(clipX(pts[i][0], W), clipY(pts[i][1], H, isMain), 0, 0); }
    const verts = new Float32Array([
        v[0], v[1], v[2], v[3],
        v[4], v[5], v[6], v[7],
        v[8], v[9], v[10], v[11],
