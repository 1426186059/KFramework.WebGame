 ...
 */
export const crCreateOffscreen = (w: number, h: number): number => {
    try {
        const gl = dom.gl;
        if (!gl) return 0;
        ensureGL();
 ...
 */
export const crSetTarget = (id: number): void => {
    try {
 ...
 */
export const setBlendProfile = (name: string): void => {
 ...
 */
export const crSetBlend = (mode: number, rate: number, enabled: boolean): void => {
    try {
        gfx.blendRate = rate;
        ...
        if (blendProfile === 'crystal') {
            ...
            switch (mode) {
                case 4:  gfx.blendOp = 'invlight'; break;  // INVLIGHT: BlendFactor / InverseSourceColor
                default: gfx.blendOp = 'lighter'; break;   // 其余模式在原版都落到 default：加色
            }
            return;
        }
        switch (mode) {
 ...
 * @param a 透明 0..255
 */
export const crClear = (r: number, g: number, b: number, a: number): void => {
    try {
        const gl = dom.gl;
        if (!gl) return;
        const tgt = gfx.cur || gfx.mainTarget;
        if (tgt.fbo) { gl.bindFramebuffer(gl.FRAMEBUFFER, tgt.fbo); gl.viewport(0, 0, tgt.w, tgt.h); }
        else { gl.bindFramebuffer(gl.FRAMEBUFFER, null); gl.viewport(0, 0, dom.canvas.width, dom.canvas.height); }
        gl.disable(gl.BLEND);
 ...
 */
export const crDraw = (tex: number, sx: number, sy: number, sw: number, sh: number, dx: number, dy: number, dw: number, dh: number, colorArgb: number): void => {
    try {
        ...
        if (gfx.blendOp === 'lighter' && gfx.blendRate !== 1) al *= gfx.blendRate;
 ...
 * @param colorArgb 颜色，高 8 位作为整体 alpha
 */
export const crDrawTransform = (tex: number, sx: number, sy: number, sw: number, sh: number, m11: number, m12: number, m21: number, m22: number, m31: number, m32: number, colorArgb: number): void => {
    try {
        const src = resolveSource(tex);
        if (!src || sw <= 0 || sh <= 0) return;
        let al = ((colorArgb >>> 24) & 0xFF) / 255;
 ...
 */
export const crMeasureText = (text: string, fontCss: string, maxWidth: number): string => {
    try {
        if (!dom.measureCtx) return '0,12';
 ...
 */
export const crFillRect = (x: number, y: number, w: number, h: number, colorArgb: number): void => {
 ...
 * @param colorArgb 颜色，32 位 ARGB
 */
export const crDrawLine = (x1: number, y1: number, x2: number, y2: number, w: number, colorArgb: number): void => {
    try {
