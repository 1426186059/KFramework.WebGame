 ...
 * @param src 源导出对象
 */
function mergeExports(target: Record<string, any>, src: Record<string, any>): void {
...
for (const name of ['MirEngine.Browser', ...(config.assemblyNames || [])] as string[]) {
    if (name === config.mainAssemblyName) continue;
    try {
...
const loadText = document.getElementById('loadText') as HTMLElement;
...
try {
    ...
    if (typeof game.GetPreloadUrls === 'function') {
        try {
            const preload: string[] = game.GetPreloadUrls();
    ...
    requestAnimationFrame(function loop(t: number): void {
        try {
            game.Frame(t);
        } catch (err) {
