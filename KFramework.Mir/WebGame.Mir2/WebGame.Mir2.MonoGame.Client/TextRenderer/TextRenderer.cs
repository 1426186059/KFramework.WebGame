...
// 文本渲染桥接。原版走 Canvas 2D（mir.cr* JS）/ 桌面端 TextRenderer(GDI)。
// 迁移到 KFramework.MonoGame 后，控件文字真实绘制走引擎字体通道（SpriteFont / SpriteBatch），
// 把文字栅格化（jsengine/text.js）后绘制进控件离屏纹理（RenderTarget2D）。
// 测量方法对齐原版 System.Windows.Forms.TextRenderer.MeasureText(text, font)，直接接收 Font。
public static class TextRenderer
{
    ...
    // 缺少时由系统字体补齐，避免「攻击 + 1」里的数字/符号显示成空白。
    ...
    // 从 css 还原出 Font（供 DrawLabel/DrawTextBox/DrawText 等 css 形态绘制 API 复用 Font 上的度量/绘制能力）
    private static Font FontFromCss(string css)
    {
        if (string.IsNullOrEmpty(css)) return new Font("Arial", 12f, GraphicsUnit.Pixel);
        css = EnsureFontFallback(css);
    ...
    // 对齐原版 TextRenderer.MeasureText：直接接收 Font，不再经 css 字符串中转。
