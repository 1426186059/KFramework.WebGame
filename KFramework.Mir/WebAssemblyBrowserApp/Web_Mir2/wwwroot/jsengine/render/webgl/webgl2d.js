...
export const drawImage = (key, sx, sy, sw, sh, dx, dy, dw, dh) => {
    ...
    if (src === undefined || src === null || sw <= 0 || sh <= 0)
        return;
...
export const drawBatch = (cmd, count) => {
    for (let i = 0; i < count; i++) {
        ...
        if (src === undefined || src === null)
            continue;
        drawTexQuad(src, cmd[o + 1], cmd[o + 2], cmd[o + 3], cmd[o + 4], cmd[o + 5], cmd[o + 6], cmd[o + 7], cmd[o + 8], 1, true);
    }
};
/**
 * 纯色填充矩形。
...
export const drawText = (text, x, y, argb, font) => {
    try {
        ...
        dom.scratch.width = tw;
        dom.scratch.height = th;
...
export const drawLabel = (handle, w, h, text, fontCss, foreArgb, outlineArgb, format, backArgb, gradTopArgb, gradBottomArgb, gradient) => {
    try {
        ...
        if (!c)
            return; // 拿不到 2D 上下文（理论上不会发生）时放弃本次绘制
        c.clearRect(0, 0, w, h);
        if (backArgb >= 0) {
            c.fillStyle = toCss(backArgb);
            c.fillRect(0, 0, w, h);
        }
        if (!text) {
        ...
        let startY;
        if (verticalCenter)
            startY = Math.max(0, (h - totalHeight) / 2);
        else if (bottom)
            startY = Math.max(0, h - totalHeight);
        else
            startY = 0;
        if (!bottom)
            startY += Math.max(1, Math.round(fs * 0.1));
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            let lx = pad;
            if (horizontalCenter)
                lx = Math.max(pad, (w - lw) / 2);
            else if (right)
                lx = Math.max(pad, w - lw - pad);
        ...
        console.error('[gl] drawLabel failed', e);
    }
};
...
export const drawTextBox = (handle, w, h, text, fontCss, foreArgb, backArgb, selBackArgb, caretArgb, selStart, selLength, caretPos, caretVisible, verticalCenter) => {
    try {
        ...
        console.error('[gl] drawTextBox failed', e);
    }
};
