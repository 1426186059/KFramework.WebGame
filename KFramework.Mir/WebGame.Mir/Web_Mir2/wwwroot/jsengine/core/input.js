...
import { dom, host, toCss } from '../shared.js';
let inputEl = null;
let areaEl = null;
let last = null; // 最近一次显示的参数，用于窗口缩放时重新定位
/**
 ...
 */
export const inputShow = (cx, cy, cw, ch, fontPx, color, value, password, maxLength, multiline) => {
 ...
 * @param ch 高度（画布像素）
 */
export const inputReposition = (cx, cy, cw, ch) => {
    if (!last)
        return;
    last.cx = cx;
    last.cy = cy;
