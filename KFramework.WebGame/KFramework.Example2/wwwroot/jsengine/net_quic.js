...
export function quicCreate(url) {
    const id = nextId++;
    let wt;
    try {
...
function acceptIncoming(handle, entry, incoming, kind) {
    (async () => {
...
function readLoop(handle, streamId, reader) {
    (async () => {
        try {
            while (true) {
                const { value, done } = await reader.read();
...
function deliver(handle, streamId, data) {
 ...
 */
export function quicOpenStream(handle, unidirectional) {
...
export function quicSend(handle, streamId, data) {
    const entry = sessions.get(handle);
    if (!entry || entry.state !== OPEN)
        return false;
    ...
    if (!stream || !stream.writer || stream.writeClosed) {
        handlers?.onError(handle, `quicSend: 流 ${id} 不可写（未就绪 / 已关闭 / 是只读的入站单向流）`);
        return false;
    }
...
export function quicCloseStream(handle, streamId) {
...
/** 中止流（立即重置，双方都不再收发）。 */
export function quicAbortStream(handle, streamId, code) {
    const stream = sessions.get(handle)?.streams.get(streamId);
    if (!stream || !stream.writer || stream.writeClosed)
        return false;
    stream.writeClosed = true;
...
export function quicClose(handle) {
    const entry = sessions.get(handle);
    if (!entry)
        return;
    entry.state = CLOSING;
...
export function quicStreamKind(handle, streamId) {
