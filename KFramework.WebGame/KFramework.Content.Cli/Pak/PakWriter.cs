...
namespace KFramework.Content.Build
{
    ...
    public sealed class PakWriter
    {
        private struct PendingEntry
        {
            ...
            public KCompressionMode Compression;
        ...
        public void Add(string name, AssetType type, ReadOnlySpan<byte> raw,
                        int width = 0, int height = 0, KCompressionMode? compression = null)
        {
            string normalized = PakFormat.NormalizeName(name);
            ulong id = PakFormat.HashId(normalized);

            KCompressionMode mode = compression ?? (raw.Length >= PakFormat.CompressionThreshold
                ? KCompressionMode.Deflate
                : KCompressionMode.None);
        ...
        /// <summary>添加一个 RGBA8 位图资源（使用 SkiaSharp SKBitmap）。</summary>
        public void AddTexture(string name, SKBitmap bitmap, AssetType type = AssetType.Texture)
        {
            using var pixmap = bitmap.PeekPixels();
            Add(name, type, pixmap.GetPixelSpan().ToArray(), bitmap.Width, bitmap.Height);
        }

        public void AddText(string name, string text, AssetType type = AssetType.Text)
            => Add(name, type, Encoding.UTF8.GetBytes(text));

        private static byte[] Compress(ReadOnlySpan<byte> raw, KCompressionMode mode)
        {
            if (mode == KCompressionMode.None || raw.Length == 0) return raw.ToArray();
