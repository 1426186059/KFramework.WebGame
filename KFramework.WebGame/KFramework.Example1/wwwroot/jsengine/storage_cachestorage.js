...
// 与 IndexedDB 相比：Cache Storage 以 Response 形式存储，专为二进制资源设计、序列化开销更小，
// 后续可叠加 Service Worker 拦截 fetch 直接返回缓存响应，做到“零解析开销”的无缝升级。
//
 ...
 * 不再写死单个缓存名：支持任意命名缓存，默认缓存名为 DEFAULT_CACHE。
...
export class Caching {
    static get DefaultName() { return DEFAULT_CACHE; }
    static _pool = new Map();
    static _current = null;

    // 打开（或获取已缓存的）一个命名缓存；同名复用同一 Cache 句柄。
    static open(name) {
        name = name || DEFAULT_CACHE;
    ...
    async loadInto(key, buffer) {
        ...
        try {
            ...
            buffer.dispose && buffer.dispose(); // ArraySegment 的视图 pin 着托管数组，用完解 pin
        }
    }
    ...
    async save(key, bytes) {
        try {
            ...
            bytes.dispose && bytes.dispose(); // ArraySegment 的视图 pin 着托管数组，用完解 pin
        }
    }
     ...
     * keep 需要保留的键（相对/绝对 URL 均可）。prefix 可选路径前缀；suffix 可选后缀。
     * 前缀与后缀是「与」关系：都给了必须同时命中才会被清理。
     */
