namespace KFramework.MonoGame
{
    ...
    ///
    /// 与原版 Crystal / UGUI 一致，本部分（TextBox.Renderer）只负责“画”；IME / 键盘捕获由 <see cref="Input_IME"/> 单独处理。
    /// 光标逻辑在 <see cref="TextBox"/> 的 TextBox.Caret 部分（见 <see cref="OnFocusChanged"/> / <see cref="Draw"/>）。
    ///
    /// <para>光标由本 TextBox 自身持有的状态自驱闪烁：传入 <paramref name="focused"/>，本方法会驱动本框的闪烁状态并绘制光标；
    /// 失焦时自动熄灭。每个 TextBox 实例各持有一份光标状态，彼此闪烁相位 / 可见性互不干扰（不再共用静态光标）。
    /// 选区高亮独立于光标，见下方 <see cref="DrawSelection"/>。</para>
    /// </summary>
    public sealed partial class TextBox
    {
        public void DrawTextBox(SpriteBatch batch, GraphicsDevice device, IFont font, string text,
            ...
            int selectionStart = 0, int selectionLength = 0)
        {
            ...
            else if (focused)
            {
                ...
                Draw(batch, device, font, display, caretPos, bounds, foreColor, multiline, padLeft);
            }
        }
